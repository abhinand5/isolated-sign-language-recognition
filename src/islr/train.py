...
def run_training(config_dict, dry_run=False, save_feature_stats=False):
    ...
    is_single_fold = (
        True
        if train_conf["K_FOLDS"] == 1 and train_conf["TRAIN_ON_ALL_DATA"]
        else False
    )
    ...
    if train_conf['LOAD_FEATURE_STATS'] is not None:
        logger.info(f"Loading feature statistics ...")
        
        with open(train_conf['LOAD_FEATURE_STATS'], 'rb') as f:
            feature_stats = pickle.load(f)

        data['feature_stats'] = feature_stats
        logger.info("Successfully loaded feature statistics!")
    else:
    ...
    if save_feature_stats:
        logger.info("Saving feature stats ...")

        with open(
            os.path.join(general_conf["BASE_DATA_DIR"], "feature_stats.pkl"), "wb"
        ) as f:
            pickle.dump(data["feature_stats"], f)

        logger.info("Successfully saved feature stats!")
